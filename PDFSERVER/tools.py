...
def get_acroform_fields(self, filename):
    fp = open(filename, 'rb')
    parser = PDFParser(fp)
...
def set_image_tag(self, filename, tag):
    exif_ifd = {
...
def get_image_tag(self, filename):
    tag = None
    try:
...
def get_images(self, filename, jpg_names, jpg_dir):
...
def get_placeholder_image_info(self, filename, xmlfile, outputdir):
...
def remove_all_images(self, filename, new_filename):
    args = [
        "gs",
        "-o",
        new_filename,
...
def repair_pdf(self, broke_pdf, fixed_pdf):
    call = [
        'pdftk',
        broke_pdf,
        'output',
...
def remove_placeholder_images(
    ...
    mynewpdf.close()
    self.repair_pdf(newpdf.replace('.pdf', '_temp.pdf'), newpdf)

def generate_fdf(self, fields, data, fdfname):
    field_value_tuples = []
    for field in fields:
...
def fill_out_form(self, fdfname, template, filledname):
...
def generate_visualizations(self, viz_files, controljs, out_dir):
...
def merge_all_pages(self, pages, final):
    call = [
        'pdftk'
    ]
    for page in pages:
...
def map_variables(self, var_list, data):
    value_list = []
...
def build_datatables(self, server_data, dt_tags):
    """
...
def build_sparklines(self, server_data, sl_tags):
    """
    serverImages = []
