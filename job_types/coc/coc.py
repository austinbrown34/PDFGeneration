...
def set_page_data(page_dataset, set_name, target, page_data=None):
    if page_data is None:
        new_page_data = {}
    else:
        new_page_data = copy.deepcopy(page_data)
    page_data[set_name] = []
    for i, e in enumerate(page_dataset):
        new_page_data[set_name].append(e)
    return page_data
...
def setup(server_data):
    ...
    print bcolors.BOLD
    print "--------------------------------------"
    print "Running Setup for Job Type: Chain of Custody"
    print "--------------------------------------"
    print bcolors.ENDC
    template_folder = value_for('lab.abbreviation', server_data)
    print bcolors.OKBLUE
    print "Preparing Chain of Custody for " + template_folder + '...'
    print bcolors.ENDC
    ...
    try:
        ...
        s3templates.download_config(
            os.path.join('coc', template_folder),
            'config.yaml',
            '/tmp/work/config.yaml'
        )
    except Exception as e:
    ...
    # print "Chain of Custody Found:"
    # print server_data['sample_coc']
    # print value_for('sample_coc', server_data)
    # template_keys = get_test_packages(value_for('sample_coc', server_data))
    template_keys = [['', 'Chain of Custody']]
    print "Finding Templates for the coc..."
    ...
    try:
        ...
        # print str(templates)
        s3templates.download_templates(os.path.join('coc', template_folder), templates)
        print "Downloading Special Scripts..."
        s3templates.download_scripts(os.path.join('coc', template_folder), scripts)
    except Exception as e:
        print "---------------------------------------"
        print str(e)
    ...
    print bcolors.BOLD
    print "--------------------------------------"
    print "Finished Preparing Data for Chain of Custody"
    print "--------------------------------------"
    print bcolors.ENDC
    # print str(data)
    return response
