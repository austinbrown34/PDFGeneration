...
def setup(server_data):
    ...
    ###### crap ######
    if 'lab_data_latest' and 'lab_data' in all_endpoints:
        server_data['lab_data'] = server_data['lab_data_latest']

    if 'cover' in all_endpoints:
    ...
    ##### adding #####
    server_data['date_received'] = value_for('date_received', all_endpoints)
    ...
    public_key = value_for('public_key', all_endpoints)
    server_data['qr_code'] = qr_base + public_profile_base + public_key

    template_folder = value_for('lab.abbreviation', all_endpoints)
    test_categories = ['cannabinoids', 'terpenes', 'solvents', 'microbials', 'mycotoxins', 'pesticides', 'metals']
    server_data['category_units'] = {}
    try:
        if 'client' in all_endpoints:
            server_data['client']['full_address'] = str(value_for('client_info.address_line_1', all_endpoints)) + ' ' + str(value_for('client_info.address_line_2', all_endpoints)) + ', ' + str(value_for('client_info.city', all_endpoints)) + ', ' + str(value_for('client_info.state', all_endpoints)) + ' ' + str(value_for('client_info.zipcode', all_endpoints))
        if 'lab' in all_endpoints:
            server_data['lab']['full_address'] = str(value_for('lab.address_line_1', all_endpoints)) + ' ' + str(value_for('lab.address_line_2', all_endpoints)) + ', ' + str(value_for('lab.city', all_endpoints)) + ', ' + str(value_for('lab.state', all_endpoints)) + ' ' + str(value_for('lab.zipcode', all_endpoints))

        server_data['batch_info'] = 'Batch #: ' + '' + '; Batch Size: ' + str(value_for('initial_weight', all_endpoints)) + ' - grams'
        server_data['initial_weight'] = str(value_for('initial_weight', all_endpoints)) + ' grams'
        ...
        if value_for('last_modified', all_endpoints) is None or value_for('last_modified', all_endpoints) == 'null':
            last_modified = ''
        else:
            last_modified = value_for('last_modified', all_endpoints).split(',')[1].split(' ')
            last_modified = last_modified[:-2]
            last_modified = last_modified[2] + ' ' + last_modified[1] + ', ' + last_modified[3]
        if value_for('date_completed', all_endpoints) is None or value_for('date_completed', all_endpoints) == 'null':
            date_completed = ''
            expires = ''
        else:
            date_completed = value_for('date_completed', all_endpoints).split(',')[1].split(' ')
            date_completed = date_completed[:-3]
            expires = value_for('date_completed', all_endpoints).split(',')[1].split(' ')
            year = value_for('date_completed', all_endpoints).split(',')[1].split(' ')
            year = year[:-2]
            year = str(year[-1])
        ...
        server_data['type_and_method'] = str(value_for('type.name', all_endpoints)) + ', ' + str(value_for('method.name', all_endpoints))
        if 'lab' in all_endpoints:
            server_data['lab']['full_street_address'] = str(value_for('lab.address_line_1', all_endpoints)) + ' ' + str(value_for('lab.address_line_2', all_endpoints))
            server_data['lab']['city_state_zip'] = str(value_for('lab.city', all_endpoints).title()) + ', ' + str(value_for('lab.state', all_endpoints)) + ' ' + str(value_for('lab.zipcode', all_endpoints))
        if 'client' in all_endpoints:
            server_data['client']['city_state_zip'] = str(value_for('client_info.city', all_endpoints).title()) + ', ' + str(value_for('client_info.state', all_endpoints)) + ' ' + str(value_for('client_info.zipcode', all_endpoints))
        server_data['special'] = {}
        server_data['date_received'] = date_received
        server_data['last_modified'] = last_modified

        r_units = value_for('lab_data.cannabinoids.report_units', all_endpoints)

        if 'client_license' in all_endpoints:
            server_data['client_license']['license_number'] = 'Lic. # ' + str(value_for('client_license.license_number', all_endpoints))

        special_cbd_total = str(value_for('lab_data.cannabinoids.cbd_total.display.' + r_units + '.value', all_endpoints)) + str(r_units)
        special_thc_total = str(value_for('lab_data.thc.thc_total.display.' + r_units + '.value', all_endpoints)) + str(r_units)
        special_moisture = str(value_for('lab_data.moisture.tests.percent_moisture.display.%.value', all_endpoints))
        server_data['special'] = {
    ...
    for category in test_categories:
        ...
        try:
            digits = value_for('lab_data.' + category + '.digits', all_endpoints)
            report_units = value_for('lab_data.' + category + '.report_units', all_endpoints)
            ...
            if category == 'cannabinoids':
                cbd_data = values_for('lab_data.cannabinoids.tests', server_data)
                thc_data = values_for('lab_data.thc.tests', server_data)
                ...
                print server_data[category + '_ordered']
                cannabinoid_data = values_for('lab_data.cannabinoids.tests', server_data)
